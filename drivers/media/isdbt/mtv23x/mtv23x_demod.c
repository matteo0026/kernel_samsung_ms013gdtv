...
#if defined(RTV_DVBT_ENABLE)
		if (g_eRtvServiceType == RTV_SERVICE_DVBT) {
			if ((cnr >= (20 * RTV_ISDBT_CNR_DIVIDER)))
				cnr = cnr - (7 * RTV_ISDBT_CNR_DIVIDER);
			else if ((cnr < (20 * RTV_ISDBT_CNR_DIVIDER))
				&& (cnr >= (10 * RTV_ISDBT_CNR_DIVIDER)))
				cnr = cnr - (3 * RTV_ISDBT_CNR_DIVIDER);
			else
				cnr = cnr + (3 * RTV_ISDBT_CNR_DIVIDER);
		}
#endif
	 ...
	 enum E_RTV_BANDWIDTH_TYPE eBandwidthType, UINT nThresholdSize)
{
	...
	U8 TMCC_L = 0;
	U8  Tcnt = 5, i = 0;
	UINT nOFDM_ReTryCnt = 30, nTMCC_ReTryCnt = 30;
...
#if 0
		if ((Mon_FSM >= 2) || (peak_pwr > pwr_threshold))
#else
	...
	if ((peak_pwr > pwr_threshold) || (Mon_FSM >= 3)) {
		for (nOFDM_LockCnt = 0; nOFDM_LockCnt < nOFDM_ReTryCnt;
							nOFDM_LockCnt++) {
			if (g_fRtv1segLpMode) {
				RTV_REG_MAP_SEL(LPOFDM_PAGE);
				RTV_REG_MASK_SET(0x13, 0x80, 0x80);
		...
		rtvMTV23x_Diversity_Path_Select(DIVERSITY_MASTER);		
			...
			if (OFDM_L) {
				...
				for (nTMCC_LockCnt = 0;
					...
					if (nTMCC_LockCnt >= (nTMCC_ReTryCnt-1)) {
						sucess_flag = RTV_CHANNEL_NOT_DETECTED;
						scan_stage = -1;
			...
			if (nOFDM_LockCnt >= (nOFDM_ReTryCnt-1)) {
				sucess_flag = RTV_CHANNEL_NOT_DETECTED;
				scan_stage = -2;
